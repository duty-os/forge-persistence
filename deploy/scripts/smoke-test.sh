...
if [ "$mode" = "app" ]; then
  ...
  [ "$code" = "403" ]
  docker compose -f docker-compose.generated.yaml -f docker-compose.override.yaml exec -T forge-persistence curl -fsS -H "X-Admin-Token: ${token}" http://127.0.0.1:3000/admin/disk/cleanup/status >/dev/null
